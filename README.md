...
- 系统设置 → 隐私与安全性 → **输入监控** → 勾选「终端」（全局快捷键 `` Ctrl+` `` 必需）

> 三个权限授权后请**完全退出并重新启动 jietu**。若用 `pythonw`/`launchd` 后台运行，授权对象是 **Python**（而非终端）。**macOS** 通过系统级 HID 监听**独占** `` Ctrl+` ``（会吞掉按键，覆盖 VS Code 终端等其它软件的同组合），必须开启「**输入监控**」和「辅助功能」后重启。
...
| 操作 | 说明 |
|------|------|
| `` Ctrl+` `` 或点击托盘图标 | 开始截图（Mac 为 Control+反引号键，全局独占，覆盖其它程序同快捷键） |
