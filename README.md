...
- 系统设置 → 隐私与安全性 → **输入监控** → 勾选「终端」（全局快捷键 `Alt+\`` / Option+` 必需）

> 三个权限授权后请**完全退出并重新启动 jietu**。若用 `pythonw`/`launchd` 后台运行，授权对象是 **Python**（而非终端）。**macOS** 通过系统级 HID 监听**独占** `Option+\``（其它程序收不到该组合键），必须开启「**输入监控**」和「辅助功能」后重启。
...
| 操作 | 说明 |
|------|------|
| `Alt+\`` 或点击托盘图标 | 开始截图（Mac 为 Option+`；Win/Mac 均**全局独占**该组合键） |
