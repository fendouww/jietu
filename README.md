...
- 系统设置 → 隐私与安全性 → **输入监控** → 勾选「终端」（全局快捷键 `Alt+\`` 必需）

> 三个权限授权后请**完全退出并重新启动 jietu**。若用 `pythonw`/`launchd` 后台运行，授权对象是 **Python**（而非终端）。**macOS 上 `Alt+\`` 不生效**时，请同时开启「辅助功能」和「输入监控」；授权后必须重启 jietu。
